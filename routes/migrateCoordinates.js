...
router.post('/migrate-coordinates', async (req, res) => {
    try {
      ...
      // Step 1: Find all warehouses that have 'address.coordinates' but do not have 'location'
      ...
      if (warehousesToMigrate.length === 0) {
        return res.status(200).json({
          message: 'No warehouses require migration.',
          migratedCount: 0,
        });
      }
      ...
      for (const warehouse of warehousesToMigrate) {
        const { coordinates } = warehouse.address; // 'coordinates' is nested under 'address'
       ...
       await warehouse.save();
          ...
          console.log(
            `Migrated Warehouse ID: ${warehouse.warehouseId} to new 'location' field.`
          );
        } else {
          console.log(
            `Invalid coordinates for Warehouse ID: ${warehouse.warehouseId}. Skipping migration for this document.`
          );
        }
      }
